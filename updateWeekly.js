...
function updateAverages(periodDocArray){
    ...
    collectionHandleWeekly.findOne({docName: "averages"}, function(err, averagesDoc) {
        ...
        if(err){ console.error(err) } 
            ...
            for(let dowIndex=0; dowIndex<averagesDoc.dowAverages.length; dowIndex++){
                var checkDowDate = new Date(periodDocArray[dowIndex].created)
                if((checkDowDate.getDay()!==0 ? (checkDowDate.getDay()-1) : 6) === dowIndex) {
                    Object.entries(periodDocArray[dowIndex].sourceTotals).forEach(([sourceName,sourceTotal])=>{
                        sourcesSoFar.set(sourceName, (sourcesSoFar.has(sourceName) ? sourcesSoFar.get(sourceName) : 0)+sourceTotal)
                    })
                    allSoFar += periodDocArray[dowIndex].allTotal
                    //now update dowAverages
                    averagesDoc.dowAverages[dowIndex].allAverage = (averagesDoc.dowAverages[dowIndex].allAverage*averagesDoc.dowAverages[dowIndex].numberOfSamples+periodDocArray[dowIndex].allTotal)/(averagesDoc.dowAverages[dowIndex].numberOfSamples+1)
                    averagesDoc.dowAverages[dowIndex].numberOfSamples++
                    Object.entries(averagesDoc.dowAverages[dowIndex].sourceAverages).forEach(([sourceName,sourceData])=>{
                        averagesDoc.dowAverages[dowIndex].sourceAverages[sourceName] = {
                            average: ((sourceData.average*sourceData.numberOfSamples+(periodDocArray[dowIndex].sourceTotals.hasOwnProperty(sourceName) ? periodDocArray[dowIndex].sourceTotals[sourceName] : 0))/(sourceData.numberOfSamples+1)),
                            numberOfSamples: sourceData.numberOfSamples+1
                        }
                    })
                    //in case we have a new source not in averages, add it
                    Object.entries(periodDocArray[dowIndex].sourceTotals).forEach(([sourceName, sourceTotal])=>{
                        if(!averagesDoc.dowAverages[dowIndex].sourceAverages.hasOwnProperty(sourceName)){
                            averagesDoc.dowAverages[dowIndex].sourceAverages[sourceName] = {
                                average: sourceTotal,
                                numberOfSamples: 1
                            }
...
function updatePeriod(periodDocArray){
    var allSoFar = 0
    var sourcesSoFar = new Map() //contains object "name"->total
    for(let dowIndex=0; dowIndex<periodDocArray.length; dowIndex++){
        var checkDowDate = new Date(periodDocArray[dowIndex].created)
        if((checkDowDate.getDay()!==0 ? (checkDowDate.getDay()-1) : 6) === dowIndex) {
            Object.entries(periodDocArray[dowIndex].sourceTotals).forEach(([sourceName,sourceTotal])=>{
                sourcesSoFar.set(sourceName, (sourcesSoFar.has(sourceName) ? sourcesSoFar.get(sourceName) : 0)+sourceTotal)
